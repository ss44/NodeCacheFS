...
function checkInit () {
  ...
  let defaults = {
    ...
    cacheDir: path.join(os.homedir(), '.fcachefs/data/'), // Path where cache blocks are stored.
    cacheBlockSize: '20mb', // Default cache block size
    dbPath: path.join(os.homedir(), '.fcachefs/db/') // path where db with meta-data is stored.
  }
  ...
  winston.debug('%s :: Loaded Options', 'init.checkInit', options)
